...
def gen_chains(out, _in, start, steps, ctx, addr):
    """
    ...
    for i in range(SPX_WOTS_LEN):
        counts[steps[i]] += 1
    total = 0
    for l in range(SPX_WOTS_W - 1, -1, -1):
    ...
    for i in range(0, SPX_WOTS_LEN, 8):
        for j in range(8):
            idx = idxs[i+j]
            addrs = set_chain_addr(addrs+j*8, idx)
